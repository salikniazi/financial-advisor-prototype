...
import { cryptoHoldings, EXCHANGE_NAME } from "@/lib/mock/crypto";
...
export default function CryptoPortfolioPage() {
  ...
  const totalCost = rows.reduce((s, r) => s + r.amount * r.avgCost, 0);
            ...
            <table className="w-full min-w-[720px] text-sm">
              <thead>
                <tr className="border-b border-border text-left text-xs uppercase tracking-wide text-muted">
                  ...
                  <th className="px-5 py-2.5 font-semibold text-right">Amount Held</th>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr
                    key={r.symbol}
                    onClick={() => router.push(`/crypto/${r.symbol}`)}
                    className="cursor-pointer border-b border-border/70 last:border-b-0 hover:bg-cream/70"
                  >
                    <td className="px-5 py-3">
                      <p className="font-bold text-ink">{r.symbol}</p>
                      <p className="text-xs text-muted">{r.name}</p>
                    </td>
                    <td className="px-5 py-3 text-right tabular-nums">{r.amount.toLocaleString("en-PK", { maximumFractionDigits: 4 })}</td>
                    ...
                    <td className="px-5 py-3 text-right tabular-nums text-ink/70">{((r.value / totalValue) * 100).toFixed(1)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
