...
import { getAIReply, suggestedPrompts, AIAction } from "@/lib/ai/respond";
...
export default function AIOverlay() {
  ...
  function send(text: string) {
    ...
    setInput("");
    setTimeout(() => {
      const reply = getAIReply(trimmed, pathname);
      setMessages((prev) => [...prev, { id: nextId(), role: "ai", text: reply.text, actions: reply.actions }]);
    }, 450);
  }
        ...
        <div className="fixed bottom-24 right-4 sm:right-6 z-40 flex h-[min(640px,calc(100vh-140px))] w-[min(400px,calc(100vw-32px))] flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-2xl">
              ...
              <div key={msg.id} className={clsx("flex", msg.role === "user" ? "justify-end" : "justify-start")}>
                <div
                  className={clsx(
                    ...
                    msg.role === "user" ? "bg-yellow text-ink rounded-br-sm" : "bg-cream text-ink rounded-bl-sm"
                  )}
                >
                  <p>{msg.text}</p>
          ...
          >
            ...
            <button
              type="submit"
              aria-label="Send"
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-ink text-yellow hover:opacity-90"
            >
              <Send size={16} />
            </button>
          </form>
